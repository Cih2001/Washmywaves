...
void ConvertWavToMP3(std::filesystem::path file_name) {
  ...
  auto number_of_channels = wave_file.GetFormatChunkHeader().number_of_channels;
  auto sample_rate = wave_file.GetFormatChunkHeader().sample_rate;
  auto bits_per_sample = wave_file.GetFormatChunkHeader().bits_per_sample;
  ...
  PRINTF("byte rate: %d\n", (int)wave_file.GetFormatChunkHeader().byte_rate);
  PRINTF("block align: %d\n",
      (int)wave_file.GetFormatChunkHeader().block_align);
  ...
  lame_set_quality(flags, 0); // let's go for the best quality baby.
